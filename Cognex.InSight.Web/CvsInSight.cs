  ...
  public async Task<object[]> QueryCellResults(string cellRange)
    ...
    private async void HandleEditorAttachedChanged(object sender, CogSocketEventArgs args)
    {
      ...
      catch (Exception ex)
      {
        ...
        {
            JObject parsedJson = JObject.Parse(_results.ToString());
            string locationToFind = cellLocation;
            JToken cells = parsedJson["cells"];
            string result = "";

            foreach (var cell in cells)
            {
                if (cell["location"].ToString() == locationToFind)
                {
                    result = cell["data"].ToString();
                    break;
                }
            }
            return result;
        ...
        public string GetCellValueByName(string cellName)
        {
            ...
            foreach (var cell in cells)
            {
                if (cell["name"] != null && cell["name"].ToString() == locationToFind)
                {
                    result = cell["data"].ToString();
                    break;
                }
